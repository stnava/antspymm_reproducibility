...
tblcols = c(  "study", "age", "sex", 'Site', 'T1Hier_resnetGrade', 'T1Hier_evratio','DTI_dti_FD_mean', 'DTI_dti_tsnr_b0_mean', 'DTI_dti_tsnr_dwi_mean', 'DTI_dti_fa_evr', 'rsfMRI_fcnxpro122_bold_evr', 'rsfMRI_fcnxpro122_FD_mean', 'rsfMRI_fcnxpro122_tsnr_mean', 'rsfMRI_fcnxpro122_ssnr_mean')
joinedem=dd[,tblcols]
...
joinedem$DTI_dti_FD_mean = joinedem$DTI_dti_FD_mean/10.
# sttbl=ztable(mytable(study~.,data=joinedem,method=3))
# sttbl=ztable(mytable(Site~.,data=joinedem,method=3,catMethod=4))
####
```
...
table1(~ . | Site, data=joinedem, topclass="Rtable1-zebra")
...
rsnames = getNamesFromDataframe( c("rsfMRI",'fcnxpro122'), dd, exclusions=c('snr','evr','FD','dvars','motion','outlier','paramset','upsampling','_mean',"_sd","left","right",'smoothing', "_nc_", "despiki","bandpass","minutes","censor") )
...
for ( a in sample(allnamesu) ) {
  n = nrow( myiccs) + 1
  tempicc = suppressWarnings(icck( ee, a ))
...
```{r rawviz,echo=FALSE,fig.width=9,fig.height=4.5}
tempnames = myiccs$anat[ !is.na( myiccs$icc ) ]
mysam = sample( tempnames[ -multigrep( c("peraf","alff"), tempnames) ], 25 )
for ( outcome in mysam ) {
  myrel = reli_stats( outcome, item="Site", id="Subject", data=ee )
  print( myrel )
...
aggM$sd=aggSD$icc
aggMf = aggM[ order( aggM$icc, decreasing=T), ]
aggMf$icc = insight::format_value( aggMf$icc, 3 )
aggMf$sd = insight::format_value( aggMf$sd, 2 )
knitr::kable( aggMf, caption='Numeric summary mean/sd ICC by type of measurement' )%>%
