...
allrsnames_cnx = getNamesFromDataframe( c("rsfMRI_fcnxpro128","Default"), ee, exclusions=c('_alff', '_falff',xcl))
...
print( myiccsrsf )
#############################################################################
mm= aggregate( icc ~ run , data=myiccsrsf[myiccsrsf$type=='cnx',], FUN=mean )
zz0=read.csv("/Users/stnava/data/PTBP/antspymm_evaluation/rsfanalysisFSIQ.csv")
zz1=read.csv("/Users/stnava/data/PTBP/antspymm_evaluation/rsfanalysisAge.csv")
zz2=read.csv("/Users/stnava/data/PTBP/antspymm_evaluation/rsfanalysisVIQ.csv")
zz=cbind(fsiq_err=zz0$test_err,viq_err=zz2$test_err, age_err=zz1$test_err,zz0,mm)
cor.test( zz$test_err, zz$icc )
summary(lm( icc ~ fsiq_err + viq_err+ age_err + ff + loop + cens + HM , data=zz))
summary(lm( icc ~ viq_err + ff + loop + cens + HM , data=zz))
