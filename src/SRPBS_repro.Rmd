...
if ( ! exists( "dd") ) {
  ...
  pym = read.csv( paste0( rdir, "/data_yeo/srpbs_travel_antspymm_yeo.csv") )[,-c(1:2)]
...
rsnames = getNamesFromDataframe( c("rsfMRI_fcnxpro122",'_2_'), dd, exclusions=c('snr','evr','FD','dvars','_mean') )
...
write.csv(myiccsrsf, paste0(rdir,'data_yeo/SRPBS_iccsrsf.csv'), row.names=FALSE)
...
myiccs = myiccs[ subtyper::fs(myiccs$icc > 0 & myiccs$icc < 1 ), ]
myiccs$modality='t1'
...
```{r comm,echo=FALSE}
write.csv(ee, paste0(rdir,'data_yeo/SRPBS_rep_comm.csv'), row.names=FALSE)
```
