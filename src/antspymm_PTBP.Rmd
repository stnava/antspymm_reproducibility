...
mth='BH'
if ( ! exists( "apym" ) ) {
  apym = read.csv( paste0(bd,'ptbp_antspymm.csv'))
  demog=read.csv(paste(bd,"dataindex/ptbp_summary_demographics.csv",sep='') )
  ...
  # PEDS022-20111216
  perfsel =  !is.na( demog$perf_gm_mean_cbf ) &
  ...
  # subtyper::fs( demog$perf_gm_mean < 0.03 & demog$perf_gm_mean > 0.001 )
  ...
  cor.test( demog$perf_gm_mean_cbf[perfsel], demog$AgeAtScan[perfsel] )
  ############################################################
  ...
  mxlprf = lmer( perf_gm_mean ~ (1|SubID) + BV + AgeAtScan + Sex, data=temp )
  rsq( mxlcbf )
  rsq( mxlprf )
  mdlcbf = lm( CBF_Frontal_Temporal_mean ~  1 + AgeAtScan + Sex, data=temp )
  mdlprf = lm( perf_gm_mean ~ 1 + AgeAtScan + Sex, data=temp )
  summary( mdlcbf )
  summary( mdlprf )

  mdlconv = lm( CBF_Frontal_Temporal_mean ~ perf_gm_mean, data=temp )
...
rsfxpt = 'rsfMRI_fcnxpro'
if ( ! exists("xptnum")) rsfxptnum = 100 else rsfxptnum=xptnum
# exclude all rsfMRI except the experiment of interest
toexclude = getNamesFromDataframe( rsfxpt, demogpym, exclusions=paste0(rsfxpt,rsfxptnum) )
demogpym = demogpym[ , ! (colnames(demogpym) %in% toexclude) ]
wthk=!is.na( demogpym$T1Hier_resnetGrade )
wcbf=perfsel & !is.na( demogpym$perf_gm_mean_cbf )
tstrsfmriname=paste0("rsfMRI_fcnxpro", rsfxptnum, "_DefaultMode_2_DefaultMode")
tstrsfmriname2=paste0("rsfMRI_fcnxpro", rsfxptnum, "_n_outliers")
tstrsfmriname3=paste0("rsfMRI_fcnxpro", rsfxptnum, "_minutes_censored_data")
...
xcl=c("_sd","_mean","FD",'motion','dvar')
...
demogpym[haveAllMerge,pfnm]=demogpym[haveAllMerge,pfnm]/demogpym$perf_gm_mean_cbf
# derka
...
# regs = regularizeSimlr( mats, fraction=0.1, sigma=rep(1.0,length(mats)) )
```
...
# derka
mymin = 10# round(min( dim(mats[[1]])) * 0.9)
nperms = 0
