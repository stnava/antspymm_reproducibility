...
# ROI and tabSIMLR reproducibility in T1w and DTI
...
if ( ! exists("dd") ) {
  demog = read.csv(paste0(rdir,"data_yeo/dti_travel_participants.csv"))
  dd = read.csv(paste0(rdir,'data_yeo/dti_travel_antspymm_yeo.csv'))[,-c(1:2)]
  names(dd)[3]='Site'
...
```{r gg2,fig.width=16,fig.height=5,echo=FALSE,eval=FALSE}
# ggthemr_reset()
...
```{r comm,echo=FALSE}
write.csv(ee,  paste0(rdir,'data_yeo/DTI_rep_comm.csv'), row.names=FALSE)
```
