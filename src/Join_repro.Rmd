...
Test-retest reliability assessment of MRI measurement is crucial for ensuring that the observed changes in the brain structure over time are due to actual physiological changes rather than variations in the imaging process itself. Traveling subject studies involve scanning the same subjects on multiple MRI scanners at different locations. These studies help in assessing consistency and/or agreement of image quantification where the only variables are the machines themselves. This is crucial for multi-site studies where data is pooled from various sources.  
...
In this study, the goal is to study the consistency of MRI IDPs across different imaging locations or scanners as processed by ANTsPyMM.

- **Multiple Raters/Instruments**: Each subject's MRI IDPs are derived from multiple MRI scanners which are considered as "raters" in this context.
- **Representation of Diverse Conditions**: These multiple measurements represent a realistic range of conditions under which MRI IDPs are obtained in practice.
- **Importance of Averaging**: The reliability of the average of measurements (IDPs from different scanners) is more relevant than individual measurements. This is because IDP interpretations are based on aggregated data from different sources in the context of clinical trials that ultimately using population-level statistical summaries.
- **Appropriateness of ICC(2,k) Model**: Lastly, ICC(2,k) is suitable for assessing the reliability of traveling cohort averaged measurements, as it considers both between-subject variability and measurement variability across different scanners. In contrast, ICC(1,k) only considers scanners as random effects while ICC(3,k) only considers subjects as random effects. ICC(2,k) considers both scanners and subjects as random effects and as such is most appropriate in this context.
...
myiccs$modality_st[asel]='rsfMRI_dfn'
...
ggpubr::ggboxplot(myiccs, y = "icc", x='modality_st',
   color = "modality", # fill = "anat",
   title='ICC variability by modality for ROI representation' )+ 
