...
rsnames = getNamesFromDataframe( c("rsfMRI_fcnxpro128"), dd, exclusions=c('_alff','_falff','snr','evr','FD','dvars') )
...
library(irr)
#####################
xcl = c( 'snr','evr','FD','dvars','outlier','minute', 'nc_','despik','motion','ermis','erebel','alff_mean','alff_sd','Unk','Subcortical') 
...
n2sam=8
ss = sample(1:length(allrsnames_fal),n2sam)
rsnames2eval = c( sample(allrsnames_cnx,n2sam), allrsnames_fal[ss], allrsnames_alf[ss] )
rsnames2evaltypes = c( rep('cnx',n2sam), rep('flf',n2sam), rep('alf',n2sam) )
...
```{r comm,echo=FALSE}
write.csv(ee, paste0(rdir,'data/Hawco_rep_comm.csv'), row.names=FALSE)
```
