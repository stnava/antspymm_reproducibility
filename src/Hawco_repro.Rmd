...
if ( ! exists("dd") )
  dd = read.csv( paste0( rdir, "/data_yeo/m3ri_travel_antspymm_yeo.csv" ) )
...
myoc = "rsfMRI_fcnxpro128_DefaultMode_2_DefaultMode"
...
rsnames = getNamesFromDataframe( c("rsfMRI_fcnxpro122"), dd, exclusions=c('_alff','_falff','snr','evr','FD','dvars','_mean') )
# rsnames = getNamesFromDataframe( c("rsfMRI_fcnxpro128",'_alff'), dd, exclusions=c('snr','evr','FD','dvars') )
...
xcl = c( 'snr','evr','FD','dvars','outlier','minute', 'nc_','despik','motion','ermis','erebel','alff_mean','alff_sd','Unk','Subcortical', 'freq','censor','smoothing','mask','peraf','PerAF','paramset','upsampling') 
...
for ( aa in rsnames2eval ) {
  ...
  for ( run in c(122,129,134) ) {
    runname = paste0("rsfMRI_fcnxpro",run)
    a = gsub( "rsfMRI_fcnxpro122", runname , aa )
    n = nrow( myiccsrsf) + 1
    if (sum(is.na(ee[,a])) == 0 ) {
      tempicc = icck( ee, a )
      ...
      if ( run %% 10 == 0 & run > 120 ) print( print( aggregate( icc ~ type , data=myiccsrsf, FUN=mean ) ) )
    }
  }
  ct = ct + 1
}
write.csv(myiccsrsf, paste0(rdir,'data_yeo/Hawco_iccsrsf.csv'), row.names=FALSE)
myiccsrsf = myiccsrsf[ !is.na( myiccsrsf$icc ) & myiccsrsf$icc > 0 ,]
print( aggregate( icc ~ run , data=myiccsrsf, FUN=mean ) )
temp = aggregate( icc ~ run , data=myiccsrsf[myiccsrsf$type=='cnx',], FUN=mean )
print( temp[order(temp$icc),] )
...
# myiccs = rbind( myiccs, myiccsrsf[myiccsrsf$run == 122,colnames(myiccs)])
myiccs$icc = as.numeric( myiccs$icc )
...
```{r comm,echo=FALSE}
write.csv(ee, paste0(rdir,'data_yeo/Hawco_rep_comm.csv'), row.names=FALSE)
```
